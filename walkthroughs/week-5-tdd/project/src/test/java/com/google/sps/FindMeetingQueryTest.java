...
public final class FindMeetingQueryTest {
  ...
  public void allDayOptionalAttendee() {
    ...
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
    // Also have an optional attendee with an event covering the entire day
  ...
  @Test
  public void morningOptionalAttendee() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
    // Also have an optional attendee with an event from 8:30 to 9:00, so that  only the start and
    // end slot should return
  ...
  public void optionalOnlyNoGaps() {
    // The only attendees are optional, and there is no schedulable time for them.
    // Thus, they should be ignored, and the whole day returned for all (none) required attendees

    Collection<Event> events =
            Arrays.asList(
                    new Event(
                            "Event 1",
                            TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_1100AM, true),
                            Arrays.asList(PERSON_A)),
                    new Event(
                            "Event 2",
